...
export function AgentNode({ id, data, selected }: NodeProps<AgentRFNode>) {
  ...
  return (
    ...
    >
      ...
      {live ? (
        <pre className="mt-2 max-h-16 overflow-hidden text-ellipsis rounded-lg border border-white/[0.06] bg-black/30 p-2 font-mono text-[10px] leading-relaxed text-slate-400">
          {live.slice(-280)}
        </pre>
      ) : null}
